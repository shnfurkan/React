...
export const changePizzasActions = (newPizzaDatas,id) => async (dispatch) => {
    const response = await axios.put(`http://localhost:8000/menu/${id}`, newPizzaDatas);
...
export const changeCartActions = (newPizzaDatas,id) => async (dispatch) => {
    const response = await axios.put(`http://localhost:8000/cart/${id}`,newPizzaDatas);
...
export const addedCartActions = (newPizzaDatas) => async (dispatch) => {
    const response = await axios.post('http://localhost:8000/cart',newPizzaDatas);
        ...
        dispatch(clearCarts());
    {
        datas.map( async (items)=> {
            if(items.quantity>0) {
                ...
                const response = await axios.put(`http://localhost:8000/menu/${items.id}`, newItemsQuantity);
